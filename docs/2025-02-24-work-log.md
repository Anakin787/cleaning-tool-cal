...
### 같은 이름 한 번만
- 폰·PC 다른 세션(익명 로그인)에서 같은 이름으로 중복 선택 방지
- 표시 이름 기준으로 기존 투표 제거 후 새 응답 반영
- `getMyScheduleResponse()`: uid 대신 표시 이름으로 "내 응답" 판단

### 선택 해제(토글)
- 참석·불참·미정 버튼 **재클릭 시 선택 해제**
- 아무것도 선택 안 된 상태로 전환
...
### 투표 설명 필드
- 투표 등록 시 **설명 (선택)** 텍스트 영역 추가
- Firestore `desc` 필드 저장
- 홈·투표 탭 카드에 설명 표시

### 투표 수정
- 투표 카드에 **수정(연필)** 버튼 추가
- 수정 시 **설명만** 수정 가능 (주제·항목·설정 변경 불가)
- `editingPollId`, `updatePollDesc()` 구현

### UI 수정
- 투표 설명 중복 표시 제거 (헤더와 본문 이중 표시 → 한 곳만)
...
### 같은 이름 한 번만 (투표)
- 폰·PC 다른 세션에서 같은 표시 이름으로 중복 투표 방지
- `getMyPollSelectedIds()`, `hasVotedByDisplayName()`: 표시 이름 기준 판단
- 투표자 목록·참여 인원 중복 이름 제거

### 선택 해제
- 단일 선택 **재클릭 시 선택 해제** (아무것도 선택 안 된 상태)
- 모바일 터치 개선: `touch-manipulation`, `min-h-[44px]`
...
### Presence
- Firestore `artifacts/{appId}/public/data/presence` 컬렉션
- 30초마다 `lastSeen` 갱신, 이탈 시 문서 삭제
- 2분 이내 활동 = 접속 중

### UI
- 헤더 "N명 접속 중" 클릭 시 접속자 이름 목록 표시
